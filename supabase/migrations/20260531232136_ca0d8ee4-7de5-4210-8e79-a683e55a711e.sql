-- Stricter matcher: no baseline 50; jobs earn points only from keyword hits.
-- Adds CJK / non-Latin language gate and cyber-domain hard requirement.
...
BEGIN
  ...
  -- Language gate: drop CJK / Cyrillic / Arabic titles when target_country is US.
  ...
  FOR f IN SELECT * FROM public.filters WHERE user_id = j.user_id LOOP
    DECLARE
      score INTEGER := 0; -- NO baseline; must earn it
      keep BOOLEAN := TRUE;
    ...
    BEGIN
      ...
      -- Hard cyber gate for cyber filters: must have title hit OR >=2 body hits
      IF keep AND is_cyber_filter AND title_kw_hits = 0 AND body_kw_hits < 2 THEN
        keep := FALSE;
      END IF;

      -- Even non-cyber filters need at least 1 keyword hit
...
-- Bump cyber filter floor so noise from low keyword counts doesn't pass
UPDATE public.filters
   SET min_score = 55
 WHERE lower(coalesce(name,'')) LIKE '%cyber%'
   AND coalesce(min_score, 0) < 55;