"""Recruitee apply ({company}.recruitee.com/*). Public hosted form."""
import time
...
class Recruitee(Portal):
    ...
    async def apply(self, *, page, job, profile, resume_pdf, cover_letter_text):
        ...
        try:
            ...
            await fill_if('input[name="candidate[name]"], input[id*="name" i]', full_name)
            await fill_if('input[name="candidate[email]"], input[type="email"]', settings().APPLY_EMAIL)
            await fill_if('input[name="candidate[phone]"], input[type="tel"]', settings().APPLY_DEFAULT_PHONE)
            ...
            try:
                ...
                await pause(0.5, 1.5)
                submit = page.locator(
                    "button[type='submit'], button:has-text('Apply'), button:has-text('Submit')"
                ).first
