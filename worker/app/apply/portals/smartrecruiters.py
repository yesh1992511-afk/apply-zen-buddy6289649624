"""SmartRecruiters apply (jobs.smartrecruiters.com/*).

Single-page form with stable field names (firstName, lastName, email, phone).
No login required.
"""
import time
...
class SmartRecruiters(Portal):
    ...
    async def apply(self, *, page, job, profile, resume_pdf, cover_letter_text):
        ...
        try:
            ...
            for sel in ['button:has-text("I\'m interested")', 'a:has-text("Apply")',
            ...
            await fill_if('input[name="firstName"], input[id*="firstName"]', first)
            await fill_if('input[name="lastName"], input[id*="lastName"]', last)
            await fill_if('input[name="email"], input[type="email"]', settings().APPLY_EMAIL)
            await fill_if('input[name="phoneNumber"], input[type="tel"]', settings().APPLY_DEFAULT_PHONE)
