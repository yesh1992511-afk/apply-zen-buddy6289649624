"""Workable apply (apply.workable.com/*). Single-page form, no login."""
import time
...
class Workable(Portal):
    ...
    async def apply(self, *, page, job, profile, resume_pdf, cover_letter_text):
        ...
        try:
            ...
            await fill_if('input[name*="first" i], input[id*="firstname" i]', first)
            await fill_if('input[name*="last" i], input[id*="lastname" i]', last)
            await fill_if('input[type="email"], input[name="email"]', settings().APPLY_EMAIL)
