...
import {
  Clock, Loader2, CheckCircle2, AlertTriangle, AlertCircle, ExternalLink, Eye,
  Mail, MessageCircle, Award, ThumbsDown, FileEdit, Inbox, RotateCcw, Trash2, Skull,
} from "lucide-react";
...
import {
  applicationsListQueryOptions, phaseOf, APPLICATIONS_QUERY_KEY,
  type ApplicationRow, type AppPhase, type AppBucket, bucketOf,
...
const HIDE_WHEN_EMPTY: AppPhase[] = [
  "follow_up_sent", "replied", "interview", "offer", "rejected", "tailored", "scored",
];

export function AllApplicationsKanban({
  bucket = "all",
  search = "",
  fullHeight = false,
}: { bucket?: AppBucket; search?: string; fullHeight?: boolean }) {
  ...
  const filtered = apps.filter((a) => {
    if (bucket !== "all" && bucketOf(a) !== bucket) return false;
    if (!s) return true;
  ...
  const byPhase = (p: AppPhase) => filtered.filter((a) => phaseOf(a) === p);
      ...
      {COLS.map(({ phase, label, icon: Icon, accent }) => {
        ...
        return (
          <div key={phase} className="flex flex-col rounded-xl border border-border/60 bg-card">
            ...
            <div className={cn("flex-1 space-y-1.5 p-2 overflow-y-auto", fullHeight ? "max-h-[70vh]" : "max-h-[480px]")}>
              {items.length === 0 ? (
